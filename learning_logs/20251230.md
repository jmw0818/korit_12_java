...
public class PersonMain {
    static void main(String[] args) {
        ...
        person1.name = "김일";
        person1.age = -10;      // 나이가 -인 상태로 입력이 가능은 함
    }
}
```
이상의 코드를 확인했을 때, 나이가 정상적이지 않은 값이 대입되어있습니다. 이처럼 외부(다른 클래스)에서
직접 속성값을 대입하게 될 때, 이상한 값이 들어가더라도 통제할 수 있는 방법이 없다는 문제가 있습니다. -> 이를 
해결하기 위한 방법이 getter / setter 개념입니다.

### 2. Getter / Setter란?
1. Setter
    - 데이터를 '설정'하는 역할을 하는 'method.' 클래스 외부에서 필드를 직접 수정하지 못하도록 막고,
    특정 메서드를 경유하여 값을 전달받아 '초기화 및 수정'을 하도록 함.
...
1. public : 모든 클래스에서 접근 가능
    - setter / getter를 alt + ins를 통해 생성하게 되면 public이라는 것을 확인할 수 있습니다.
    애초에 다른 클래스에서 사용하라고 만든 것이기 때문에 기본적으로 public에 해당합니다.
