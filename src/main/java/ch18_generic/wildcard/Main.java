...
public class Main {
    ...
    /*
        ...
        아닌지를 구분할 수 없기 때문에 이를 판단하는 메서드를 Main에서 정의
     */
    ...
    public static void main(String[] args) {
        ...
        /*
            이상의 상황에서는 AnimalData에 Car 객체가 animal field에 들어가있지만 문제가 되지 않음.
            그렇다면 animal field에 Tiger / Human / Car 객체가 들어가있다는 점은
            여기 main에서 확인이 안됨. 코드 다른 곳을 확인해봐야 함.
         */
        animalData1.showData();
        animalData2.showData();
//        animalData3.showData();   // 오류 발생
        /*
            AnimalData 클래스를 확인하면, (Animal)이라고 형변환을 시기키는 부분 때문인데,
            Car를 Animal로 형변환 시킬 수 있는 근거가 없음.
         */
        // .getAnimal() 메서드 호출
        Main main = new Main();
        // wildcard 적용
        ...
        if (animalData100 == null) {
            ...
            System.out.println("객체는 있는데 값이 없습니다.");
        }
